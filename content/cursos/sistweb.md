---
title: Sistemas Web (Mestrado)
date: "2026-02-04"
---

[Dia, Hora e Local](#dia-hora-e-local)\
[Objetivo](#objetivo)\
[Tópicos](#tópicos)\
[Bibliografia](#bibliografia)\
[Avaliação](#avaliação)\
[Cronograma](#cronograma)\
[Material](#material)\
[Sites para Pesquisa](#sites-para-pesquisa)

## Dia, Hora e Local

3as, 19-22h

## Objetivo

Apresentar tópicos de pesquisa relacionados à web. Para tal, faremos uma revisão dos conceitos básicos da web, introduziremos tópicos de pesquisa com apresentações, implementação de trabalhos práticos e discussões de artigos. 

## Tópicos

- Arquitetura Web
- Common Gateway Interface
- Programação em Rede
- Linguagens de Programação no Cliente (Navegador)
- Tipos de Escopo
- Persistência e Rastreamento de Clientes
- Linguagens de Programação no Servidor (Servidor Web)
- Modelagem de Sistemas Web
- Padrões de Aplicações Web
- Frameworks de Aplicações Web
- Metalinguagem XML e algumas linguagens afins
- Enriquecimento de Aplicações Web (RIA)
- Acessibilidade na Web
- Web Services
- Definição de APIs
- Recuperação de Dados na Web
- Mineração de Dados na Web
- Big Data
- Linked Data
- Linked Open Data e Semântica na Web
- Agentes de IA

## Bibliografia

- Robert W. Sebesta, Programming the World Wide Web, Addison Wesley.
- James F. Kurose, Keith W. Ross, Computer Networking, Addison Wesley.
- Tom Heath and Christian Bizer, [Linked Data: Evolving the Web into a Global Data Space](http://linkeddatabook.com/editions/1.0/), Morgan & Claypool.
- Ricardo Baeza-Yates, Berthier Ribeiro-Neto, Modern Information Retrieval, Addison Wesley.
- [W3Schools](http://www.w3schools.com/)
- [Curso de FullStack da Universidade de Helsinki](https://fullstackopen.com/en/)

## Avaliação

Média final = Média-Trabalhos

## Cronograma

|Assunto|Dia|
| ----- |---|
| Apresentação, Critérios de Avaliação e Histórico |10/mar|
| Web API's                                        |17/mar|
| Recuperação e Mineração de Dados                 |24/mar|
| Semântica na Web                                 |31/mar|
| Protocolos e Agentes Web                         |07/abr|
|                                                  |14/abr|
| *Feriado* |21/abr|
|       |28/abr|
|       |05/mai|
|       |12/mai|
|       |19/mai|
|       |26/mai|

## Material

- [Introdução à Prog. Web](ProgramacaoWeb.pdf)
- [XML](XML.pdf)
- [Web API's](WebAPIs.pdf)
- [Web Semântica](WebSemantica.pdf)
- [RDF](RDF.pdf)
- [Vocabularios](Vocabularios.pdf)
- [Mineração na Web](WebMining.pdf)
- [Big Data](BigData.pdf)

## Sites para Pesquisa

- [Periodicos Capes](http://www.periodicos.capes.gov.br)
- [Science Direct](http://www.sciencedirect.com)
- [ACM](http://dl.acm.org/dl.cfm)
- [IEEExplore](http://ieeexplore.ieee.org/Xplore/home.jsp)
- [Google Scholar](https://scholar.google.com.br/)
- [arXiv e-Print archive (Kept by Cornell University)](https://arxiv.org/)
- [Scielo](http://www.scielo.org/)
- [Scopus](https://www.scopus.com/pages/home)
